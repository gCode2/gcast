...
<section id="createContent" class="section border-0 bg-dark overlay overlay-show overlay-op-8 overflow-hidden m-0" style="height: max-content;">
					...
					<div class="box-content">
											<h4 class="color-primary font-weight-semibold text-7 mb-3 text-center">Edit podcast!</h4>
											<form action="{$conf->action_root}editView&id={$podcast["id"]}" id="frmSignIn" method="post" class="needs-validation" novalidate="novalidate" enctype="multipart/form-data">
												<div class="row">
                                                    <div class="form-group col">
														<label class="form-label">Change title</label>
														<input type="text" name="title" value="{$podcast["title"]}" class="form-control form-control-lg" minlength="3" required="">
													</div>
												</div>
                                                <div class="row">
													<div class="form-group col">
														<label class="form-label">Change description (500 characters allowed)</label>
														<textarea class="form-control" aria-label="Description" maxlength="500" name="description">{$podcast["description"]}</textarea>
													</div>
												</div>
                                                <div class="row">
													<div class="form-group col">
                                                    <label for="category">Change category</label>
														<select id="category" class="form-select form-control h-auto py-2" data-msg-required="Please select a category." name="category" required>
                                                                ...
                                                                <script>
                                                                    document.getElementById('category').selectedIndex={$podcast["category_id"]};
                                                                </script>
													</div>
												</div>
												<div class="row">
													<div class="form-group col">
														...
														<input type="file" name="podcast" id="podcast" class="form-control form-control-lg" accept="audio/*">
													</div>
                                                    
												</div>
												<div class="row justify-content-center">
													<div class="form-group col-lg-6 join-section">
														<button type="submit" class="btn btn-outline btn-rounded btn-with-arrow mb-2 login">Edit podcast!</button>
													</div>
												</div>
											</form>
										</div>
